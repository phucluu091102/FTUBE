import React from "react";
import Box from '@mui/material/Box';
import { DataGrid, defaultColumnOptions } from '@mui/x-data-grid';
import { Button, Typography } from "@mui/material";
import { DetailPro } from "../Detail/detailPro";
import { Link } from 'react-router-dom';
const columnOptions = {
    defaultColumnOptions: {
        rowHeight: 550
    }
};


const columns = [
    {
        field: 'id', headerName: 'Sản Phẩm', width: 200, height: 200,
        renderCell: (params) => <Box sx={{ width: '100px', height: '96px', backgroundColor: 'red' }}> <img src={"data:image/png;base64,"}
            style={{ width: '100px', height: '96px' }} /></Box>


    },
    {
        field: 'tieude',
        headerName: 'Tiêu đề',
        width: 250,
        editable: true,
        height: 200,
    },
    {
        field: 'tensinhvien',
        headerName: 'Tên sinh viên',
        width: 250,
        editable: true,
        height: 200,
    },
    {
        field: 'chitiet',
        headerName: 'Chi tiết',
        // type: 'number',
        width: 120,
        // editable: true,
        LectureItem: null,
        url: './detailProduct',
        renderCell: (cellValue) => {
            return (
                <Button variant="outlined"
                    color="primary"
                    component={Link} to="/detailProduct"

                // onClick={(event) => {
                //     handleClick(event, cellValue);
                // }}
                >
                    Chi tiết
                </Button>


            )
        }


    },
    {
        field: 'trangthai',
        headerName: 'Trạng thái',
        width: 170,


        renderCell: (cellValue) => {
            return (
                <>
                    <Box sx={{
                        display: 'flex',
                        justifyContent: 'center', // Căn giữa theo chiều ngang
                        alignItems: 'center',
                        width: "100px", height: "35px", backgroundColor: "#EE0000", color: "white", fontSize: 15,
                        borderRadius: 1
                    }}>
                        TỪ CHỐI
                    </Box>

                    {/* <Button variant="contained"
                        color="error"
                    >
                        Thất bại
                    </Button> */}
                    {/* <Button variant="contained"
                        color="damage"
                    >
                        Từ chối
                    </Button> */}


                </>
            )
        }
    },




];


const rows = [
    { id: 1, tensinhvien: 'Lưu Đình Thiên Phúc', tieude: 'jv6' },
    { id: 2, tensinhvien: 'Võ Thị Thanh Na', tieude: 'datn' },
    { id: 3, tensinhvien: 'Trần Nhựt Anh', tieude: 'jv6' },
    { id: 4, tensinhvien: 'Trần Trung Thịnh', tieude: 'datn' },
    { id: 5, tensinhvien: 'Nguyễn Phương Đông', tieude: 'jv6' },
    { id: 6, tensinhvien: 'Phan Minh Hồng Phúc', tieude: 'jv6' },
    { id: 7, tensinhvien: 'Hàng Dũng Linh', tieude: 'jv5' },
    { id: 8, tensinhvien: 'Bùi Đoàn Khuê', tieude: 'jv6' },
    { id: 9, tensinhvien: 'Mai Phi Long', tieude: 'datn' },
];
export const Refuse = () => {
    return (
        <Box sx={{ height: 600, width: '100%' }}>
            <div>
                <h1>Danh sách Từ chối</h1>
            </div>
            <DataGrid
                rows={rows}
                columns={columns}
                {...columnOptions}
                getEstimatedRowHeight={() => 200}
                getRowHeight={() => 'auto'}
                row
                initialState={{
                    pagination: {
                        paginationModel: {
                            pageSize: 5,
                        },
                    },
                }}


                pageSizeOptions={[5]}
                // checkboxSelection
                disableRowSelectionOnClick
            />


        </Box>
    )
}







