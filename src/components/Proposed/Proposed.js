import React from "react";
import Box from '@mui/material/Box';
import { DataGrid } from '@mui/x-data-grid';
import { Button } from "@mui/material";
import { Link, useNavigate } from 'react-router-dom';
import { ReasonRefuse } from "./Refuse/reasonrefuse";

const columnOptions = {
    defaultColumnOptions: {
        rowHeight: 550
    }
};
const button = [
    { id: 1, name: 'button1' },
    { id: 2, name: 'button2' },
]


const columns = [
    {
        field: 'id', headerName: 'Sản Phẩm', width: 200, height: 200,
        renderCell: (params) => <Box sx={{ width: '100px', height: '96px', backgroundColor: 'red' }}> <img src={"data:image/png;base64,"}
            style={{ width: '100px', height: '96px' }} /></Box>



    },
    {
        field: 'tieude',
        headerName: 'Tiêu đề',
        width: 250,
        editable: true,
        height: 200,
    },
    {
        field: 'tensinhvien',
        headerName: 'Tên sinh viên',
        width: 250,
        editable: true,
        height: 200,
    },
    {
        field: 'chitiet',
        headerName: 'Chi tiết',
        // type: 'number',
        width: 120,
        // editable: true,
        renderCell: (cellValue) => {
            return (
                <Button variant="outlined"
                    color="primary"
                    component={Link} to="/detailProduct"
                // onClick={() => viewDetails(rowData)}

                >

                    Chi tiết
                </Button>
            )
        }
    },
    {
        field: 'trangthai',
        headerName: 'Trạng thái',
        width: 250,
        renderCell: (cellValue) => {
            return (
                <>
                    <Button variant="contained"
                        color="success"
                        sx={{ marginRight: 1 }}
                        component={Link} to="/accept"
                    >
                        Đồng ý
                    </Button>
                    <Button variant="contained"
                        color="error"
                        component={Link} to="/resonRefuse"
                    // onClick={handleOpenModal}
                    >
                        Từ chối
                    </Button>
                </>
            )
        }
    },



];
const rows = [
    { id: 1, tensinhvien: 'Lưu Đình Thiên Phúc', tieude: 'jv6' },
    { id: 2, tensinhvien: 'Võ Thị Thanh Na', tieude: 'datn' },
    { id: 3, tensinhvien: 'Trần Nhựt Anh', tieude: 'jv6' },
    { id: 4, tensinhvien: 'Trần Trung Thịnh', tieude: 'datn' },
    { id: 5, tensinhvien: 'Nguyễn Phương Đông', tieude: 'jv6' },
    { id: 6, tensinhvien: 'Phan Minh Hồng Phúc', tieude: 'jv6' },
    { id: 7, tensinhvien: 'Hàng Dũng Linh', tieude: 'jv5' },
    { id: 8, tensinhvien: 'Bùi Đoàn Khuê', tieude: 'jv6' },
    { id: 9, tensinhvien: 'Mai Phi Long', tieude: 'datn' },
    { id: 10, tensinhvien: 'Lưu Đình Thiên Phúc', tieude: 'jv6' },
    { id: 12, tensinhvien: 'Võ Thị Thanh Na', tieude: 'datn' },
    { id: 13, tensinhvien: 'Trần Nhựt Anh', tieude: 'jv6' },
    { id: 14, tensinhvien: 'Trần Trung Thịnh', tieude: 'datn' },
    { id: 15, tensinhvien: 'Nguyễn Phương Đông', tieude: 'jv6' },
    { id: 16, tensinhvien: 'Phan Minh Hồng Phúc', tieude: 'jv6' },
    { id: 17, tensinhvien: 'Hàng Dũng Linh', tieude: 'jv5' },
    { id: 18, tensinhvien: 'Bùi Đoàn Khuê', tieude: 'jv6' },
    { id: 19, tensinhvien: 'Mai Phi Long', tieude: 'datn' },
];

function Proposed() {
    // const history = useNavigate();

    // // Hàm xử lý sự kiện khi nhấn vào button
    // const viewDetails = (rowData) => {
    //     // Chuyển đến trang mới và truyền dữ liệu
    //     history.push('/detailProduct', rowData);
    // };
    // const [isOpenModal, setIsOpenModal] = useState(false);
    // const handleOpenModal = () => setIsOpenModal(true);
    // const handleCloseModal = () => setIsOpenModal(false);
    return (

        <Box sx={{ height: 600, width: '100%' }}>
            <div>
                <h1>Danh sách Đề xuất</h1>
            </div>
            <DataGrid
                rows={rows}
                columns={columns}
                {...columnOptions}
                getEstimatedRowHeight={() => 200}
                getRowHeight={() => 'auto'}
                row
                initialState={{
                    pagination: {
                        paginationModel: {
                            pageSize: 5,
                        },
                    },
                }}


                pageSizeOptions={[5]}
                // checkboxSelection
                disableRowSelectionOnClick
            />
            {/* <UploadProjectModalContext.Provider value={{ isOpenModal, handleCloseModal }}>
                <ReasonRefuse />
            </UploadProjectModalContext.Provider> */}
        </Box>

    )
}


export default Proposed;
